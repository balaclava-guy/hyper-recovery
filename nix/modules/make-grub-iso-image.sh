# Custom ISO image builder for unified GRUB bootloader
# This is based on NixOS make-iso9660-image.sh but removes the /isolinux reference
# that causes xorriso to fail when using GRUB instead of syslinux.
...
if test -n "$bootable"; then
    ...
    # Use GRUB boot directory for sort-weight instead of isolinux
    isoBootFlags="-eltorito-boot ${bootImage}
                  -eltorito-catalog .boot.cat
                  -no-emul-boot -boot-load-size 4 -boot-info-table
                  --sort-weight 1 /boot/grub" # Make sure GRUB is near the beginning of the ISO
fi
