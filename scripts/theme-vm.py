...
def _make_fat_theme_drive_image(mformat: str, mcopy: str, share_dir: Path, out_img: Path) -> None:
    # Create a small FAT image with a stable volume label so the guest can mount it via:
    #   /dev/disk/by-label/HYPERTHEME
    #
    # This avoids relying on QEMU's fat:rw pseudo-drive, which has no stable label and
    # tends to force manual mounting/debugging.
    out_img.parent.mkdir(parents=True, exist_ok=True)
    if out_img.exists():
        out_img.unlink()

    # 256 MiB is plenty for these theme assets.
    with out_img.open("wb") as f:
        f.truncate(256 * 1024 * 1024)

    _run([mformat, "-i", str(out_img), "-v", "HYPERTHEME", "::"])

    # Copy each top-level entry so we don't depend on shell globbing behavior.
    for item in sorted(share_dir.iterdir(), key=lambda p: p.name):
        _run([mcopy, "-s", "-i", str(out_img), str(item), "::/"])
            ...
            write_files:
              - path: /usr/local/sbin/hyper-theme-sync
                ...
                content: |
                  ...
                  find_theme_dev() {
                    if [[ -e /dev/disk/by-label/HYPERTHEME ]]; then
                      readlink -f /dev/disk/by-label/HYPERTHEME
                      return 0
                    fi
...
def _write_nixos_module(dest: Path) -> None:
    ...
    dest.write_text(
        textwrap.dedent(
            """\
            # Minimal NixOS module to apply the GRUB + Plymouth themes from /etc/nixos/hyper-theme-src.
            #
            ...
            let
              ...
              # Must exist at nixos-rebuild evaluation time. hyper-theme-sync keeps this in /etc/nixos.
              themeSrcPath = /etc/nixos/hyper-theme-src;
              haveThemeSrc = builtins.pathExists themeSrcPath;
              themeSrc = if haveThemeSrc then themeSrcPath else null;

              # Known password ("nixos") for the dev VM.
              # sha512crypt hash generated with: openssl passwd -6 -salt hyper-theme nixos
              pwHash = "$6$hyper-theme$Hv7WUOJ8Z215tv2aZTJIxG10sPxyQ/V2NTUvrornk9AXtH8X0n7SlN5L9LssQVQ2pj5vJGofm3hUgt1zglKqa0";

              grubTheme = if haveThemeSrc then (pkgs.stdenvNoCC.mkDerivation {
              ...
              plymouthTheme = if haveThemeSrc then (pkgs.stdenvNoCC.mkDerivation {
              ...
              hyperThemeSync = pkgs.writeShellScript "hyper-theme-sync" ''
                ...
                mnt=/mnt/hyper-theme
                mkdir -p "$mnt"
                # If a fileSystems entry exists (automount), this will mount on first access.
                if [ -e /dev/disk/by-label/HYPERTHEME ] && [ ! -e "$mnt/THEME_SYNC_MARKER" ]; then
                  true
                fi
                if [ ! -e "$mnt/THEME_SYNC_MARKER" ]; then
                ...
                ${pkgs.coreutils}/bin/rm -rf /etc/nixos/hyper-theme-src
                ${pkgs.coreutils}/bin/mkdir -p /etc/nixos/hyper-theme-src
                ${pkgs.coreutils}/bin/cp -R "$mnt/themes" /etc/nixos/hyper-theme-src/
                ${pkgs.coreutils}/bin/cp -R "$mnt/assets" /etc/nixos/hyper-theme-src/

                # Keep the NixOS module itself up-to-date from the host repo.
                ${pkgs.coreutils}/bin/mkdir -p /etc/nixos
                ${pkgs.coreutils}/bin/cp "$mnt/nixos/hyper-theme.nix" /etc/nixos/hyper-theme.nix
            ...
            in
            {
              # Make SSH + console usable immediately, with known credentials.
              users.mutableUsers = false;
              users.users.root.hashedPassword = pwHash;
              users.users.theme = {
                isNormalUser = true;
                extraGroups = [ "wheel" ];
              ...
              services.getty.autologinUser = "theme";
              ...
              # In a VM we can safely use EFI variables; this makes GRUB the default boot entry
              # (and avoids getting stuck in a previous systemd-boot entry stored in the vars file).
              ...
              boot.loader.grub = {
                ...
                efiSupport = true;
                device = "nodev";
                theme = lib.mkIf haveThemeSrc grubTheme;
              ...
              boot.plymouth.theme = lib.mkIf haveThemeSrc "snosu-hyper-recovery";
              boot.plymouth.themePackages = lib.mkIf haveThemeSrc [ plymouthTheme ];

              # Auto-mount the theme drive for debugging convenience. (hyper-theme-sync also
              # knows how to mount it directly if needed.)
              fileSystems."/mnt/hyper-theme" = {
                device = "/dev/disk/by-label/HYPERTHEME";
                fsType = "vfat";
                options = [ "ro" "nofail" "x-systemd.automount" ];
              };

              # Keep Plymouth visible for theme iteration by delaying plymouth-quit.
              # Avoid `environment.etc` drop-ins here; they can conflict with other etc entries.
              systemd.services.plymouth-quit.serviceConfig.ExecStartPre =
                [ "${pkgs.coreutils}/bin/sleep ${toString holdSeconds}" ];
...
def _write_nixos_install_script(dest: Path) -> None:
    dest.write_text(
        textwrap.dedent(
            ...
            rm -rf /mnt/etc/nixos/hyper-theme-src
            mkdir -p /mnt/etc/nixos/hyper-theme-src
            cp -R "$theme_mnt/themes" /mnt/etc/nixos/hyper-theme-src/
            cp -R "$theme_mnt/assets" /mnt/etc/nixos/hyper-theme-src/

            nixos-install --no-root-passwd

            # Ensure we can actually log in (console + SSH) even if activation scripts
            # don't run in the way we expect during install.
            nixos-enter --root /mnt -c 'echo "root:nixos" | chpasswd'
            nixos-enter --root /mnt -c 'echo "theme:nixos" | chpasswd' || true
...
def _write_nixos_install_readme(dest: Path) -> None:
    dest.write_text(
        textwrap.dedent(
            ...
            Default credentials (installed system):
              user: theme
              pass: nixos
              ssh:  ssh -p 2222 theme@127.0.0.1
            """
        ),
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    theme_drive_img = cache_dir / "theme-drive.img"
    ...
    mformat = os.environ.get("MFORMAT", "@mformat@")
    ...
    if mformat.startswith("@"):
        mformat = _which("mformat") or ""
    if mcopy.startswith("@"):
        mcopy = _which("mcopy") or ""
    ...
    if not mformat or not mcopy:
        sys.stderr.write("Missing mtools (mformat/mcopy) in PATH.\n")
        sys.stderr.write("If using the flake package, add nixpkgs.mtools to the environment.\n")
        return 1
    ...
    # Create a labeled FAT image for the share drive (so the guest can mount it automatically).
    _make_fat_theme_drive_image(mformat, mcopy, share_dir, theme_drive_img)
    ...
    if args.base == "ubuntu":
        ...
        if install_mode:
            qemu_args.extend(
                [
                    # Boot the installer ISO only once; after install, the reboot should go to disk.
                    "-boot",
                    "once=d",
                    "-drive",
    ...
    # Attach the theme drive last so it doesn't become /dev/vda.
    qemu_args.extend(["-drive", f"if=virtio,format=raw,file={theme_drive_img}"])
